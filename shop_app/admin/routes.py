...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return redirect(url_for('login'))
    return render_template('admin/register.html', form=form, title='hairForDays | Register')
...
def login():
    ...
    if request.method == "POST" and form.validate():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash(
                f"Welcome Back '{form.email.data}', you're logged in", 'success')
            return redirect(request.args.get('next') or url_for('admin'))
        else:
            flash('Incorect email or password, try again', 'danger')
    return render_template('admin/login.html', form=form, title='hairForDays | Signin')
